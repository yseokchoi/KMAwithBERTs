...
class BeamSearch(DecodeStrategy):
    ...
    def advance(self, log_probs, log_sep_probs, attn):
        ...
        if log_sep_probs is not None:
            _, max_sep_ids =torch.max(log_sep_probs, dim=-1)
            self.alive_sep = torch.cat(
                [self.alive_sep.index_select(0, self.select_indices),
                 max_sep_ids.unsqueeze(1)], -1
            )
    ...
    def update_finished(self):
        ...
        sep_predictions = self.alive_sep.view(_B_old, self.beam_size, step) if self.alive_sep.size(-1) == step else None
        attention = (
        ...
        for i in range(self.is_finished.size(0)):
            ...
            if finish_flag and len(self.hypotheses[b]) >= self.n_best:
                ...
                for n, (score, pred, attn, sep) in enumerate(best_hyp):
                    if n >= self.n_best:
                        break
                    ...
                    if sep is not None:
                        self.sep_predictions[b].append(sep)
            else:
        ...
        self.alive_sep = sep_predictions.index_select(0, non_finished).view(-1, self.alive_sep.size(-1)) \
            if sep_predictions is not None else self.alive_sep
